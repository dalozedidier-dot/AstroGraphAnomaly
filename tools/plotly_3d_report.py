...
def plot_star_cloud(df: pd.DataFrame, out_html: Path) -> bool:
    ...
    rr = np.log1p(dist[m])  # compress depth
    xyz = uv * rr.reshape(-1, 1)

    s = score_hi(df.iloc[np.where(m)[0]])
    c = robust_01(s)
    ...
    fig = go.Figure(
        data=[
            go.Scatter3d(
                ...
                text=hover_text(df.iloc[np.where(m)[0]]),
                hoverinfo="text",
            )
        ]
    )
    fig.update_layout(
        title="Star cloud 3D (log(distance) scaled)",
...
def plot_celestial_sphere(df: pd.DataFrame, out_html: Path) -> bool:
...
def plot_graph_3d(G: nx.Graph, df: pd.DataFrame, out_html: Path, title: str) -> None:
    ...
    if {"source_id", "anomaly_score"}.issubset(df2.columns):
        for sid, sc in zip(df2["source_id"].tolist(), pd.to_numeric(df2["anomaly_score"], errors="coerce").tolist(), strict=False):
            if isinstance(sc, float) and math.isfinite(sc):
                score_map[str(sid)] = float(sc)
    if {"source_id", "anomaly_label"}.issubset(df2.columns):
        for sid, lb in zip(df2["source_id"].tolist(), pd.to_numeric(df2["anomaly_label"], errors="coerce").fillna(1).tolist(), strict=False):
            try:
    ...
    node_trace = go.Scatter3d(
        x=x, y=y, z=z,
        mode="markers",
        marker=dict(size=sizes, opacity=0.88, color=colors, colorscale="Viridis"),
        text=htxt,
        hoverinfo="text",
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    plot_star_cloud(df, out_dir / "01_star_cloud_xyz.html")
    plot_celestial_sphere(df, out_dir / "02_celestial_sphere.html")

    gt = run_dir / "graph_topk.graphml"
    if gt.exists():
        Gt = nx.read_graphml(gt)
        plot_graph_3d(Gt, df, out_dir / "03_graph_topk_3d.html", title="Graph top-k 3D (sky-space embedding)")

    gf = run_dir / "graph_full.graphml"
    if gf.exists():
        Gf = nx.read_graphml(gf)
        plot_graph_3d(Gf, df, out_dir / "04_graph_full_3d.html", title="Graph full 3D (sky-space embedding)")
