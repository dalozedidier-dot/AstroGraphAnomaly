...
def _ensure_plotlyjs(out_dir: Path) -> None:
    """Make Plotly HTML self-contained without CDN.

    When include_plotlyjs="directory", Plotly expects a plotly.min.js file
    next to the HTML. We write it once per output directory.
    """
    js_path = out_dir / "plotly.min.js"
    try:
        if js_path.exists() and js_path.stat().st_size > 100_000:
            return
    except Exception:
        pass
    try:
        ...
        # If Plotly is not installed, nothing to do.
        return
